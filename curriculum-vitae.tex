...
\begin{adjustwidth}{-1.5cm}{}
    \begin{tikzpicture}
        ...
        % Draw the categories
        \node[category] (web) at (-5,0) {Desarrollo Web};
        \node[category-smaller] (desktop) at (2,2) {Escritorio};
        \node[category-smaller, align=center, text depth=1.5cm] (mobile) at (5,3) {Móvil};
        \node[category-small] (db) at (-8,-8.5) {Bases de datos};
        \node[category-small] (vcs) at (-1,-9) {Control de versiones};
        \node[category-small] (tools) at (5,-9) {Herramientas};
        \node[subcategory] (dotnet) at (-0.5,2.5) {\includesvg[scale=2]{csharp.svg}};

        % Draw the skills within the categories
        \node[skill] (html) at (-8,1.5) {{\fontsize{30pt}{30pt}\faHtml5}\\\textbf{HTML}};
        \node[skill] (css) at (-5.5,1.5) {{\fontsize{30pt}{30pt}\faCss3*}\\\textbf{CSS}};
        \node[skill] (js) at (-3.8,0) {{\fontsize{30pt}{30pt}\faJs}\\\textbf{JavaScript}};
        \node[skill] (ts) at (-1.8,-1.7) {{\fontsize{30pt}{30pt}\includesvg[scale=1.3]{typescript.svg}}\\\textbf{TypeScript}};
        \node[skill] (node) at (-3.7,-3.2) {{\fontsize{30pt}{30pt}\faIcon{node-js}}\\\textbf{Node.js}};
        \node[skill] (php) at (-6.25,-1.25) {\fontsize{40pt}{40pt}\faPhp};
        \node[skill,text width=2.5cm] (wordpress) at (-8.5,-1.5) {{\fontsize{30pt}{30pt}\faWordpress}\\\textbf{WordPress}};
        \node[skill] (elementor) at (-6.3,-3.2) {{\fontsize{30pt}{30pt}\faElementor}\\\textbf{Elementor}};
        \node[skill] (docker) at (-1,-5) {{\fontsize{30pt}{30pt}\faDocker}\\\textbf{Docker}};
        \node[skill-circle, text width=1.5cm] (wpf) at (1.3,3) {WPF};
        \node[skill] (unity) at (0.9,1.3) {{\fontsize{30pt}{30pt}\faUnity}\\\textbf{Unity}};
        \node[skill-circle, text width=2.3cm] (aspnetcore) at (-1.5,1.5) {ASP.NET Core};
        \node[skill] (flutter) at (5.2,2.25) {{\fontsize{30pt}{30pt}\includesvg[scale=1.1]{flutter.svg}}\\Flutter};
        \node[skill] (sql) at (-9,-8) {{\fontsize{30pt}{30pt}\faDatabase}\\SQL};
        \node[skill] (firebase) at (-7,-8) {{\fontsize{30pt}{30pt}\includesvg[scale=1.2]{firebase.svg}}\\Firebase};
        \node[skill] (git) at (-1,-8) {{\fontsize{30pt}{30pt}\faGit}};
        \node[skill] (github) at (1,-8) {{\fontsize{30pt}{30pt}\faGithub}\\GitHub};
        \node[skill] (vscode) at (-1.5,-10.5) {{\fontsize{30pt}{30pt}\includesvg[scale=1.1]{visual-studio.svg}}\\Visual Studio};
        \node[skill] (vscode) at (1.5,-10.5) {{\fontsize{30pt}{30pt}\includesvg[scale=1.1]{vscode.svg}}\\VS Code};
        \node[skill] (trello) at (0,-13) {{\fontsize{30pt}{30pt}\faTrello}\\Trello};
    \end{tikzpicture}
\end{adjustwidth}
